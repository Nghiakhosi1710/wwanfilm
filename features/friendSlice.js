...
export const getFriends = createAsyncThunk("friends/getFriends", async (userId, { rejectWithValue }) => {
    try {
        // API backend trả về { success: true, data: { friends, friendRequests, ... } }
        const response = await axios.get(`${API_URL}/${userId}`, { headers: authHeader() });
        if (response.data?.success) {
            return response.data.data; // Chỉ trả về phần data chứa các danh sách
        } else {
            // Ném lỗi nếu success không phải true hoặc không có data
            throw new Error(response.data?.message || 'Failed to get friends data');
        }
    } catch (error) {
        // Trả về lỗi từ response hoặc message lỗi chung
